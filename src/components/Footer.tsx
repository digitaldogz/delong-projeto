...
const Footer = () => {
  ...
  return (
    <footer id="footer" className="w-full bg-background text-foreground border-t border-border">
      ...
      {/* ========================================= */}
      <div className="hidden md:block container-premium">
        <div className="grid grid-cols-2 gap-16 py-24">
          {/* LADO ESQUERDO */}
          <div className="flex flex-col justify-between gap-12">
            <div>
              <h3 className="text-3xl font-bold tracking-tight mb-4">A arte move tudo</h3>
            ...
            <a href={`https://wa.me/${config.whatsapp_number}`} target="_blank" rel="noreferrer" className="group block w-full max-w-md bg-foreground hover:bg-foreground/90 transition-colors py-6 px-8 flex justify-between items-center">
              <span className="text-xl font-bold text-background tracking-tight">Tem um projeto? Vamos conversar</span>
          ...
          {/* LADO DIREITO */}
          <div className="flex flex-col justify-between border-l border-border pl-16">
            ...
            <div className="mt-12 grid grid-cols-2 gap-8">
              <div>
                ...
                <a href={`mailto:${config.contact_email}`} className="text-lg font-bold hover:text-muted-foreground transition-colors">{config.contact_email}</a>
              </div>
              <div className="text-right mt-0">
                <span className="text-xs text-muted-foreground uppercase tracking-widest block mb-3">TELEFONE/WHATSAPP</span>
                <a href={`https://wa.me/${config.whatsapp_number}`} className="text-lg font-bold hover:text-muted-foreground transition-colors">{config.whatsapp_number}</a>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ========================================= */}
      {/* MOBILE FOOTER (Zeitmedia Layout) */}
      {/* ========================================= */}
      <div className="md:hidden container-premium py-12">
        <div className="flex flex-col gap-8 mb-12">
          <div className="max-w-md">
            <h3 className="text-3xl font-medium tracking-tight mb-4">A arte move tudo</h3>
        ...
        <div className="flex justify-center gap-12 py-12 border-b border-border">
        ...
        <div className="grid grid-cols-1 gap-8 py-12">
          ...
          <div>
            ...
            <a href={`mailto:${config.contact_email}`} className="text-xl font-light hover:text-muted-foreground transition-colors">{config.contact_email}</a>
          </div>
          <div>
            <span className="text-xs text-muted-foreground block mb-2">WhatsApp / Telefone</span>
            <a href={`https://wa.me/${config.whatsapp_number}`} className="text-xl font-light hover:text-muted-foreground transition-colors">{config.whatsapp_number}</a>
          </div>
        </div>
      </div>
